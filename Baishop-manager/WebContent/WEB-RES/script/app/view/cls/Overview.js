 ...
 */
Ext.define('Baishop.view.cls.Overview', {
    extend: 'Ext.panel.Panel',
    alias: 'widget.classoverview',
    requires: [
        'Baishop.view.cls.Toolbar',
        'Baishop.Syntax',
        'Baishop.Settings'
    ],
    ...
    load: function(docClass) {
        ...
        this.toolbar = Ext.create('Baishop.view.cls.Toolbar', {
            docClass: this.docClass
        });
        this.addDocked(this.toolbar);

        this.update(this.renderClass(docClass));
        Baishop.Syntax.highlight(this.getEl());

        if (Baishop.Settings.get("hideInherited")) {
    ...
    renderLink: function(className) {
        return Ext.String.format('<a href="#/{0}" rel="{0}" class="docClass">{0}</a>', className);
    },
    ...
    renderMemberDiv: function(member, index) {
        this.memberTpl = this.memberTpl || new Ext.XTemplate(
            '<div id="{tagname}-{name}" class="member {firstChild} {inherited}">',
                ...
                '<div class="title">',
                    '<div class="meta">',
                        '<a href="#/{owner}" rel="{owner}" class="definedIn docClass">{owner}</a><br/>',
    ...
    renderSignature: function(member) {
        this.signatureTpl = this.signatureTpl || new Ext.XTemplate(
            '{before}<a href="#/{member}-{tagname}-{name}" class="name {expandable}">{name}</a>{params}{after}'
        );
